...
/**
* Json library
* @class Business_unit_controller
...
class Business_unit_controller {
    ...
    function read() {
        ...
        $sidx = getVarClean('sidx','str','bu.listingno');
        $sord = getVarClean('sord','str','asc');

        $data = array('rows' => array(), 'page' => 1, 'records' => 0, 'total' => 1, 'success' => false, 'message' => '');

        $parentid_fk = getVarClean('parentid_fk','int',0);
        $businessgroupid_fk = getVarClean('businessgroupid_fk','int',0);

        try {

            $ci = & get_instance();
            $ci->load->model('data_master/business_unit');
            $table = $ci->business_unit;
            ...
            if(!empty($businessgroupid_fk)) {
                $req_param['where'][] = 'businessgroupid_fk = '.$businessgroupid_fk;
            }
    ...
    function readLov() {
        ...
        $sort = getVarClean('sort','str','bu.listingno');
   ...
   function crud() {
    ...
    public function tree_json() {
        ...
        $businessgroupcode = getVarClean('businessgroupcode','str','System');
        $businessgroupid_fk = getVarClean('businessgroupid_fk','int',-999);

        $table->setCriteria('businessgroupid_fk = '.$businessgroupid_fk);
        $items = $table->getAll(0,-1,'listingno','asc');
        $data = array();
        $data[] = array('id' => 0,
                  'parentid' => -1,
                  'text' => $businessgroupcode,
                  'expanded' => true,
                  'selected' => true,
        ...
        foreach($items as $item) {
            ...
            if( $table->emptyChildren($item['businessunitid_pk']) ) {
                $data[] = array(
                            'id' => $item['businessunitid_pk'],
